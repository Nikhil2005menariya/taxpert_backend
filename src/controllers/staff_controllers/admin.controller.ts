...
export const getAssignments = async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await service
      .from('ca_assignments')
      .select(`
        ca_id, client_id, assigned_at,
        ca:users!ca_assignments_ca_id_fkey(first_name, last_name, pan, role, email, mobile),
        client:users!ca_assignments_client_id_fkey(first_name, last_name, pan, email, mobile)
      `)
...
export const assignToClient = async (req: Request, res: Response) => {
  try {
    ...
    const { assigneeId, clientId } = req.body;

    const service = createServiceClient();
    const { error } = await service.from('ca_assignments').upsert({ ca_id: assigneeId, client_id: clientId });
    if (error) return res.status(400).json({ error: error.message });

    res.json({ success: true });
  } catch (error) {
    console.error('assignToClient error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const removeAssignment = async (req: Request, res: Response) => {
  try {
    ...
    const { caId, clientId } = req.query;

    const service = createServiceClient();
    const { error } = await service.from('ca_assignments').delete().eq('ca_id', caId as string).eq('client_id', clientId as string);
    if (error) return res.status(400).json({ error: error.message });
